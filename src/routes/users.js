...
usersRouter.get("/", async (req, res) => {
    try {
      ...
      console.log(error);
      res.status(500).json({ message: "Internal server error", success: false, error: error }); //in case of error responding with 500
    }
  });
...
usersRouter.get("/:id", async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        res.status(404).json({ message: "user with this id does not exists" }); //if user not found responding with 404
      } else {
        res.send(user); //if user found sending user as response
      }
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Internal server error", success: false, error: error }); //in case of error responding with 500
...
usersRouter.post("/register", postValidator, async (req, res) => {
  ...
  //Updating the data of an existing user 
usersRouter.put("/:id", putValidator, async (req, res) => {
